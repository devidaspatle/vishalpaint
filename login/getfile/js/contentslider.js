...
for(var i=0;i<alldivs.length;i++){if(this.css(alldivs[i],"contentdiv","check")){setting.contentdivs.push(alldivs[i])}}if(setting.currentpage>setting.contentdivs.length){setting.currentpage=1
...
var toclinkscount=0
for(var i=0;i<pdivlinks.length;i++){if(this.css(pdivlinks[i],"toc","check")){if(toclinkscount>setting.contentdivs.length-1){continue}pdivlinks[i].setAttribute("rel",++toclinkscount)
...
this.fadeup(setting)}if(setting.enablefade[0]==false){setting.contentdivs[setting.prevpage-1].style.display="none"}setting.contentdivs[turntopage-1].style.visibility="visible"
...
window["fcsfade"+setting.id]=setTimeout(function(){featuredcontentslider.fadeup(setting)},50)}else{if(setting.cacheprevpage!=setting.currentpage)setting.onChange(setting.cacheprevpage,setting.currentpage)}},cleartimer:function(setting,timervar){if(typeof timervar!="undefined"){clearTimeout(timervar)
clearInterval(timervar)
if(setting.cacheprevpage!=setting.currentpage){}}},css:function(el,targetclass,action){var needle=new RegExp("(^|\\s+)"+targetclass+"($|\\s+)","ig")
